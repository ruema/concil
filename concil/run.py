...
    #pid = os.fork()
    pid = libc.syscall(CLONE, SIGCHLD | CLONE_NEWUSER|CLONE_NEWNS|CLONE_NEWPID, None, None, None, None)
    if pid < 0:
...
            self.image_config = json.load(file)
        self.config = image_config.get('config', {})
...
        # remove all LD_-Variables like LD_LIBRARY_PATH or LD_PRELOAD
    ...
    def working_dir(self):
        return config.get('WorkingDir') or "/"
...
            commandline.extend(args)
    ...
    def get_layers(self):
        return [
            os.path.join(self.basepath, l.split(':')[1])
