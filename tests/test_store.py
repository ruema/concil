...
def test_store_init(mocker, mock_config):
    ...
    mocker.patch("concil.store.complete_url_with_auth", return_value=store.parse_docker_url("docker://docker.io/library/alpine:latest"))
    mocker.patch("concil.store.get_full_url", return_value="https://registry.hub.docker.com/v2/library/alpine")
    ...
    store.DockerHub.assert_called_once_with("https://registry.hub.docker.com/v2/library/alpine", verify=None)
...
def test_store_get_manifest_no_trust(mocker, mock_config):
    ...
    mock_hub.get_manifest.return_value = b'{"mediaType": "application/vnd.docker.distribution.manifest.v2+json"}'

    s = store.Store("docker://docker.io/library/alpine:latest", config=mock_config)
    manifest = s.get_manifest()

    mock_hub.get_manifest.assert_called_once_with(accept="application/vnd.docker.distribution.manifest.v2+json")
...
def test_store_get_manifest_with_notary(mocker, mock_config):
    ...
    mocker.patch("concil.store.complete_url_with_auth", return_value=store.parse_docker_url("docker://docker.io/library/alpine:latest"))
    mocker.patch("concil.store.get_full_url", return_value="https://registry.hub.docker.com/v2/library/alpine")
    mocker.patch("concil.store.get_notary_url", return_value="https://notary.docker.io/library/alpine")
    mocker.patch('getpass.getpass', return_value='password')
    mocker.patch('builtins.input', return_value='user')
