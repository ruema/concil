...
import tarfile
from concil.streams import _Stream, GZipStream, DirTarStream, MergedTarStream
...
def test_mergetar():
    ...
    with tarfile.open(fileobj=BytesIO(result)) as tar:
        assert {(m.name, m.size) for m in tar.getmembers()} == {('a', 3), ('b', 3), ('c/.wh..wh..opq', 0), ('c/d', 1), ('d', 1)}